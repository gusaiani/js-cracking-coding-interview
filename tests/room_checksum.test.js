...
test('a-b-c-d-e-f-g-h-987[abcde] is a real room', () => {
  expect(checkRoom('a-b-c-d-e-f-g-h-987[abcde]')).toEqual(
    {"id": 987, "isReal": true}
  );
});

test('not-a-real-room-404[oarel] is a real room', () => {
  expect(checkRoom('not-a-real-room-404[oarel]')).toEqual(
    {"id": 404, "isReal": true}
  );
});

test('totally-real-room-200[decoy] is not a real room', () => {
  expect(checkRoom('totally-real-room-200[decoy]')).toEqual(
    {"id": 200, "isReal": false}
  );
});

test('enzcntvat-pnaql-qrfvta-351[antqv] is not a real room', () => {
  expect(checkRoom('enzcntvat-pnaql-qrfvta-351[antqv]')).toEqual(
    {"id": 351, "isReal": true}
  );
});

test ('molgbzqfib-bdd-mrozexpfkd-289[bdfmo] is a real room', () => {
  expect(checkRoom('molgbzqfib-bdd-mrozexpfkd-289[bdfmo]')).toEqual(
    {"id": 289, "isReal": true}
  );
})

test('sum sector IDs of Real Rooms', () => {
  const rooms = [
